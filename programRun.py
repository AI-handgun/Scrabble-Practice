...
def run(width=1000, height=600):
    ...
    def mousePressedWrapper(event, canvas, data):
        if x.data.endOfGame:
            ...
            maxCombo = EV.maxComboValue(workingCombos, boardKeeper.board, occupied)
            if maxCombo[0] != -1:
            ...
            x.data.message2 = "Your score is "+str(round(humanPlayer.points/maxCombo[0]*100, 2))+"% of the computer score"
    ...
    for i in range(225):
        if i not in data.occupiedBoardLocations:
            data.emptyBoardLocations.append(i)  # fills this with every location
    ...
    humanPlayer.addToHand(removedLetters)
    x.data.letterHand = removedLetters
    removedLetters = letterBag.removeLetters(7)
    computerPlayer.addToHand(removedLetters)
