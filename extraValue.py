...
def calcWordValue(combo, letterDict, board, occupied):
    wordValue = 0
    timesDouble = 0
    timesTriple = 0
    
    for spot in combo:
        if spot in doubleWord and spot not in occupied:
            timesDouble += 1
        elif spot in tripleWord and spot not in occupied:
            timesTriple += 1
        if spot in doubleLetter and spot not in occupied:
            if spot in letterDict.keys():
                wordValue += 2 * dictionary[letterDict[spot]]
            else:
                wordValue += 2 * dictionary[board[spot]]
        elif spot in tripleLetter and spot not in occupied:
            if spot in letterDict.keys():
...
def maxComboValue(workingCombos, board, occupied):
    maxCombo = [-1, [], []]
    maxLocations = []
    for workingCombo in workingCombos:
        ...
        for combo in workingCombo[2]:
            ...
            comboValue += calcWordValue(combo, letterDict, board, occupied)
